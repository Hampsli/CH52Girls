...
def addmultiplenumbers():
    numeros = obtener_numeros("Ingresa los números que quieres sumar(separados con comas): ")
    return sum(numeros) #Suma todos los numeros que fueron ingresados y los regresa a la funcion.

def multiplymultiplenumbers():
    numeros = obtener_numeros("Ingresa los números que quieres multiplicar (separados con comas): ")
    resultado = 1
...
def es_numero(valor):
    ...
    if valor.isdigit():  # Si es un número entero
        return True
...
def calculadora():
    ...
    while True:
        ...
        if opcion == "1":
            ...
            print(f"El número {num} {'es par' if isiteven(num) else 'no es par'}.")

        elif opcion == "4":
            num = obtener_numeros("Ingrese un número para verificar si es entero: ")[0]
            print(f"El número {num} {'es entero' if isitaninteger(num) else 'no es entero'}.")
